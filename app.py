...
class InterceptHandler(logging.Handler):
    def emit(self, record):
        ...
        while frame.f_code.co_filename == logging.__file__:
            frame = frame.f_back
            depth += 1
        logger.opt(depth=depth, exception=record.exc_info).bind(logtype=record.name).log(level, record.getMessage())
...
def main(loglevel, cfg_file):
    ...
    #####
    cfg.load(cfg_file)
    server_dir = Path(cfg.get('server.directory', cfg.path.parent)).expanduser().resolve()
    ...
    if loglevel != 'DISABLE':
        ...
        debugstr = '<cyan>{name}</cyan>:<cyan>{function}</cyan>:<cyan>{line}</cyan> - ' if debug else ''
        logger.add(sys.stderr,
            level = loglevel.name,
            format = '<light-black>{time:YYYY-MM-DD HH:mm:ss}</light-black> | <level>' + levelstr + '</level> | {extra[logtype]} | ' + debugstr +'{message}',
            filter = None,
            backtrace = debug,
            diagnose = debug
        )
    ...
    if loglevel != 'DISABLE':
        ...
        logger.add(logdir / 'app-{time:YYYY-MM-DD}.log',
            level = loglevel.name,
            format = '{time:YYYY-MM-DD HH:mm:ss.SSS} | {level: <8} | {extra[logtype]} | ' + debugstr + '{message}',
            filter = None,
            backtrace = debug,
            diagnose = debug,
            enqueue = True,
    ...
    with logger.catch(onerror=lambda _: sys.exit(1)):
        ...
        protocol = 'https' if (ssl_keyfile is not None and ssl_certfile is not None) else 'http'
